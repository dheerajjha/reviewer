...
async function loadReviewDocument(reviewsDir, repoPath, { file = null } = {}) {
  const { comments, mode, range } = await readSavedComments(reviewsDir, repoPath);
  ...
  const document = filterReviewDocument(
    buildReviewDocument({ repoPath, comments, generatedAt: new Date(), head, branch, mode, range }),
    { file }
  );
