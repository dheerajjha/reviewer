...
function parseCommitFilePath(raw) {
  ...
  if (braceMatch) {
    ...
    const normalized = rawTo.replace(/\/{2,}/g, '/');
    return { isRename: true, path: unquoteGitPath(normalized) };
  }
  ...
  if (arrowMatch) {
    ...
    return { isRename: true, path: unquoteGitPath(rawTo) };
  }

  return { isRename: false, path: unquoteGitPath(raw) };
}
