 ...
 * @param {string} repoPath
 * @returns {Promise<{comments: object[], mode: string|null, range: object|null, adopted: boolean}>}
...
async function readSavedComments(reviewsDir, repoPath) {
  ...
  if (saved !== null) {
    ...
    return { comments: saved.comments ?? [], mode: saved.mode ?? null, range: saved.range ?? null, adopted: false };
  }

  const legacyName = legacyCommentsFilename(repoPath);
  if (legacyName === commentsFilename(repoPath)) return { comments: [], mode: null, range: null, adopted: false };

  const legacy = await readJson(path.join(reviewsDir, legacyName));
  if (legacy === null || !belongsTo(legacy.repoPath, repoPath)) {
    return { comments: [], mode: null, range: null, adopted: false };
  }
  ...
  return { comments: legacy.comments ?? [], mode: legacy.mode ?? null, range: legacy.range ?? null, adopted: true };
}
