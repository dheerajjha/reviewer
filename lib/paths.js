 ...
 *
 * For display only — nothing resolves a `~` back, and nothing should. An
 * absolute path is the right thing to store, to compare and to hand to git;
 * it is the wrong thing to put in a list of projects, where the first forty
 * characters are the same on every row and the part that identifies the row
 ...
 * @returns {string}
 */
function homeRelative(target, home = os.homedir()) {
